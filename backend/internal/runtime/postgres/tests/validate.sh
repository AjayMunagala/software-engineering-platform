...
powershell.exe -NoProfile -NonInteractive -Command \
    "\$env:POSTGRES_TEST_URL='postgres://postgres@127.0.0.1:${cluster_port}/${database_name}?sslmode=disable'; \$env:AEGIS_RUNTIME_POSTGRES_INTEGRATION='1'; \$env:AEGIS_REPOSITORY_SERVICE_INTEGRATION='1'; \$env:AEGIS_RUNTIME_POSTGRES_PORT='$cluster_port'; \$env:AEGIS_RUNTIME_POSTGRES_DATABASE='$database_name'; \$env:AEGIS_RUNTIME_POSTGRES_USER='$runtime_login'; \$env:AEGIS_RUNTIME_POSTGRES_INGEST_USER='$ingest_login'; \$env:AEGIS_RUNTIME_POSTGRES_READ_USER='$read_login'; \$env:AEGIS_RUNTIME_POSTGRES_RETENTION_USER='$retention_login'; \$env:AEGIS_RUNTIME_POSTGRES_ROOT_CA='$ca_windows'; Set-Location -LiteralPath '$backend_windows'; go test ./internal/storage/postgres -run '^TestPostgresNeutralConformance$' -count=1; if (\$LASTEXITCODE -ne 0) { exit \$LASTEXITCODE }; go test ./internal/runtime/postgres ./internal/runtime/app -cover -count=1; if (\$LASTEXITCODE -ne 0) { exit \$LASTEXITCODE }; \$integrationOutput = go test ./internal/service/repository/integration -cover -count=1 -v 2>&1; \$integrationCode = \$LASTEXITCODE; \$integrationOutput | Select-String 'repository_service_metrics|^--- PASS|^PASS|^ok '; if (\$integrationCode -ne 0) { \$integrationOutput | Select-Object -Last 80; exit \$integrationCode }"
